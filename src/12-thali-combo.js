...
export function createThaliDescription(thali) {
  if (typeof thali !== "object" || thali === null || Array.isArray(thali) || typeof thali.name !== "string" || !Array.isArray(thali.items) || typeof thali.price !== "number" || typeof thali.isVeg !== "boolean") return "";
...
export function searchThaliMenu(thalis, query) {
  ...
  return thalis.filter(thali => {
    const nameMatch = thali.name.toLowerCase().includes(lowQuery);
    const itemMatch = thali.items.some(item => item.toLowerCase().includes(lowQuery));
